...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    function addFilaGroup(uid) {
      ...
      try {
        var c = getClient(uid);
        if (!c) { engine.log("[Mix] addFilaGroup: cliente nao encontrado para " + uid); return; }
        var groups = c.getServerGroups();
        for (var i = 0; i < groups.length; i++) {
          if (parseInt(groups[i].id()) === FILA_GID) {
            engine.log("[Mix] Cliente ja tem cargo FILA");
            return;
          }
        }
        c.addToServerGroup(FILA_GID);
        engine.log("[Mix] Cargo FILA adicionado para " + c.name());
      } catch(e) {
    ...
    function removeFilaGroup(uid) {
      ...
      try {
        var c = getClient(uid);
        if (!c) { engine.log("[Mix] removeFilaGroup: cliente nao encontrado para " + uid); return; }
        c.removeFromServerGroup(FILA_GID);
        engine.log("[Mix] Cargo FILA removido de " + c.name());
      } catch(e) {
