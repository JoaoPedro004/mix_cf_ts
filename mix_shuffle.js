  ...
  function (sinusbot, config) {
    ...
    function createMatchChannel(name, afterChannelId) {
      try {
        ...
        if (afterChannelId) {
          // Cria depois de um canal específico (ex: depois do Time 1)
          props.order = afterChannelId;
        } else if (config.stopBeforeChannelId) {
          // Cria antes do spacer
    ...
    event.on("chat", function(ev) {
      ...
      if (cmd === P + "mix") {
        ...
        // Inicia um mix de teste com menos jogadores (mínimo 2)
        if (sub === "filateste") {
          ...
          if (mainQueue.status !== "open") {
            client.chat("❌ A fila principal não está aberta. Use !mix reset para liberar.");
            return;
          }

          // Pega clientes online (exceto bot) para preencher a fila de teste
          var onlineClients = backend.getClients().filter(function(c) { return !c.isSelf(); });

          if (onlineClients.length < 2) {
            client.chat("❌ Precisa de pelo menos 2 jogadores online para o teste.");
            return;
          }

          // Usa quantos tiver disponível até o limite pedido
          var testPlayers = onlineClients.slice(0, testSize);

          // Salva o NEEDED original e muda temporariamente
          var originalNeeded = NEEDED;
          mainQueue.players = testPlayers.map(function(c) { return c.uid(); });
          mainQueue.status = "closed";

          broadcast(
            "🧪 [ADMIN] " + client.name() + " iniciou uma FILA DE TESTE com " +
            testPlayers.length + " jogador(es)!"
          );

          // Força o NEEDED para o tamanho do teste
          var oldNEEDED = NEEDED;
          NEEDED = testPlayers.length;
          startMatch(mainQueue);
          NEEDED = oldNEEDED;

          client.chat("✅ Fila de teste iniciada com " + testPlayers.length + " jogador(es)!");
          return;
        }
