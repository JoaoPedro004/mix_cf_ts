      ...
      if (cmd === P + "mix") {
        ...
        if (sub === "update") {
          ...
          require("child_process").exec(
            "curl -s -o " + SCRIPT_PATH + " " + GITHUB_URL,
            function (err, stdout, stderr) {
              if (err) {
                client.chat("❌ Erro ao baixar atualização: " + err);
                return;
              }
              client.chat("✅ Script atualizado! Reiniciando o bot em 3 segundos...");
              broadcast("🔄 [ADMIN] " + client.name() + " atualizou o bot. Reiniciando...");
              setTimeout(function () {
                require("child_process").exec("systemctl restart sinusbot", function(){});
              }, 3000);
            }
          );
          return;
