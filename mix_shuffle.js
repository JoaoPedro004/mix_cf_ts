...
registerPlugin(
  {
    ...
    vars: [
      ...
      { name: "logMatchChannelId", title: "Canal LOG DE PARTIDAS (resultados, filas, vetos)", type: "channel" },
      { name: "logAdminChannelId", title: "Canal LOG ADMIN/BAN (bans, resets, comandos admin)", type: "channel" },
  ...
  function (sinusbot, config) {
    ...
    // bannedList: { uid: { until: timestamp|null, reason: string } }
    var bannedList = store.get("bannedList") || {};
    function saveBanned() { store.set("bannedList", bannedList); }

    // duration: null = permanente, ou ms
    function banPlayer(uid, durationMs, reason) {
      var until = durationMs ? (Date.now() + durationMs) : null;
      bannedList[uid] = { until: until, reason: reason || "sem motivo" };
      saveBanned();
      return true;
    }
    function unbanPlayer(uid) {
      if (bannedList[uid]) { delete bannedList[uid]; saveBanned(); return true; }
      return false;
    }
    function isBanned(uid) {
      var entry = bannedList[uid];
      if (!entry) return false;
      if (entry.until === null) return true; // permanente
      if (Date.now() < entry.until) return true; // ainda no prazo
      // Ban expirou — remove automaticamente
      delete bannedList[uid];
      saveBanned();
      return false;
    }
    function banExpiry(uid) {
      var entry = bannedList[uid];
      if (!entry) return null;
      return entry.until; // null = permanente
    }
    function banReason(uid) {
      var entry = bannedList[uid];
      return entry ? entry.reason : "";
    }
    function parseBanDuration(str) {
      if (!str) return null; // permanente
      var n = parseInt(str);
      if (isNaN(n) || n <= 0) return null;
      var unit = str.replace(/[0-9]/g, "").toLowerCase();
      if (unit === "h") return n * 3600000;
      if (unit === "d") return n * 86400000;
      if (unit === "m") return n * 2592000000; // 30 dias
    ...
    // ─── Log de Canais ────────────────────────────────────────
    // logMatch → canal de partidas (resultados, filas, vetos, lado)
    // logAdmin → canal admin/ban (bans, resets, limpar ranking, forçar)

    function logToChannel(channelId, msg) {
      if (!channelId) return;
      try {
        var ch = getChannel(channelId);
        if (ch) ch.chat(msg);
      } catch(e) {
        engine.log("[Mix] Erro ao logar no canal " + channelId + ": " + e);
      }
    }

    function logMatch(msg) { logToChannel(config.logMatchChannelId, msg); }
    function logAdmin(msg) { logToChannel(config.logAdminChannelId, msg); }

    // ─── Histórico de Ranking ─────────────────────────────────
    function saveRankingHistory(triggeredBy) {
      var r = getRanking();
      var list = [];
      ...
      var histKey = "rankingHistory_" + now.getFullYear() + "_" + (now.getMonth()+1);
      var entry = {
        date: dateStr,
        by: triggeredBy,
        total: matchCounter,
      ...
      store.set(histKey, entry);
      engine.log("[Mix] Histórico de ranking salvo em " + histKey + " por " + triggeredBy);
      return entry;
    }

    function clearRanking(adminName, isAuto) {
      saveRankingHistory(isAuto ? "reset-auto" : adminName);
      store.set('ranking', {});
      updateRankingChannel();
      var msg = isAuto
        ? "[RESET] Ranking zerado automaticamente (virada do mês). Histórico salvo."
        : "[RESET] [ADMIN] " + adminName + " zerou o ranking. Histórico salvo.";
      broadcast(msg);
      logAdmin("[" + (new Date()).toLocaleString('pt-BR') + "] " + msg);
    }

    // ─── Reset automático no 1º do mês ───────────────────────
    (function scheduleMonthlyReset() {
      function checkMonthlyReset() {
        var now = new Date();
        if (now.getDate() === 1 && now.getHours() === 0 && now.getMinutes() < 1) {
          var lastReset = store.get("lastMonthlyReset") || "";
          var key = now.getFullYear() + "-" + (now.getMonth()+1);
          if (lastReset !== key) {
            store.set("lastMonthlyReset", key);
            clearRanking(null, true);
            logAdmin("[AUTO] Reset mensal de ranking executado em " + now.toLocaleDateString('pt-BR') + ".");
            engine.log("[Mix] Reset mensal do ranking executado.");
          }
        }
      }
      // Verifica a cada 60 segundos
      setInterval(checkMonthlyReset, 60000);
    })();
    ...
    function applyResult(winnerUids, loserUids) {
      ...
      loserUids.forEach(function(u) {
        ...
        r[u].pts    = Math.max(0, r[u].pts + RANK_LOSS); // pontos não ficam negativos
        r[u].losses += 1;
      });
      saveRanking(r);
      updateRankingChannel();
    ...
    function buildRankingText() {
      ...
      list.forEach(function(p, i) {
        ...
        lines.push(prefix + p.name + '  |  ' + p.pts + ' pts  (' + p.wins + 'V / ' + p.losses + 'D)');
      });
      lines.push('=================================');
    ...
    function updateRankingChannel() {
      ...
      if (!ch) return;
      try {
        ch.setDescription(buildRankingText());
      } catch(e) {
        engine.log("[Mix] Erro ao atualizar descricao do canal ranking: " + e);
      }
    }
    ...
    function getBotClient() {
      var cs = backend.getClients();
      for (var i = 0; i < cs.length; i++)
        if (cs[i].isSelf()) return cs[i];
      return null;
    }

    function botGoToFila() {
      setTimeout(function() {
        var bot = getBotClient();
        if (bot) {
          bot.moveTo(config.filaChannelId);
          engine.log("[Mix] Bot voltou para o canal da fila.");
        } else {
          engine.log("[Mix] getBotClient() retornou null!");
        }
      }, 500);
    }
    ...
    function pokeAdmins(msg) {
      backend.getClients().forEach(function(c) {
        if (!c.isSelf() && isAdmin(c)) c.poke("[URGENTE] [ADMIN] " + msg);
      });
    }
    ...
    function updateFilaName() {
      ...
      try {
        var n = queue.length;
        // Simbolos Unicode basicos (3 bytes, suportados pelo V8 do SinusBot)
        // U+25CF = bola preenchida, U+25CB = bola vazia
        // Nao ha como mudar cor no nome do canal no TS3 via texto
        var dot;
        if (n === 0)       dot = "\u{1F534}";  // bola vermelha
        else if (n <= 4)   dot = "\u{1F534}";  // bola vermelha
        else if (n <= 7)   dot = "\u{1F7E1}";  // bola amarela
        else               dot = "\u{1F7E2}";  // bola verde

        var label = n >= NEEDED ? n + "/" + NEEDED + " COMPLETA" : n + "/" + NEEDED;
        var suffix = dot + " Fila de Espera [ " + label + " ]";
        if (matches.length > 0) {
          suffix += " | " + matches.length + " partida(s)";
        }
      ...
      while (queue.length >= NEEDED) {
        ...
        pickedUids.forEach(function(u) {
          ...
          try {
            ...
            if (pc2) {
              var pn = pc2.name();
              if (pn.indexOf("[NA FILA] ") !== -1) pc2.setNickname(pn.replace("[NA FILA] ", ""));
            }
          } catch(e) {}
        });
        ...
        logMatch("[" + tag + "] INICIADA | Time1(Cap:" + nameOf(slot.cap1) + "): " + t1p.map(function(c){ return c.name(); }).join(", ") + " | Time2(Cap:" + nameOf(slot.cap2) + "): " + t2p.map(function(c){ return c.name(); }).join(", "));
    ...
    function startVeto(slot) {
      ...
      toCapChannel(slot,
        "---------------------------------\n" +
        "[MAPA]  VETO DE MAPAS  —  " + tag + "\n" +
        listaTxt + "\n" +
        "---------------------------------\n" +
        ">> VEZ DE: " + nameOf(capUid) + "\n" +
        "   Use: " + P + "vetar <número>\n" +
        "[TEMPO] " + nameOf(capUid) + " tem 3 minutos."
      );
      ...
      slot.vetoTimer = setTimeout(function() {
        ...
        toCapChannel(slot, "[TIMEOUT] Tempo esgotado! Bot vetou automaticamente: " + slot.maps[idx]);
    ...
    function processVeto(slot, voterUid, mapIdx) {
      ...
      toCapChannel(slot,
        "[X] " + nameOf(voterUid) + " vetou: [b]" + vetoed + "[/b]\n" +
        "   Restam " + slot.maps.length + " mapa(s)."
      );

      logMatch("[FILA#" + slot.matchNum + "] VETO | " + nameOf(voterUid) + " vetou: " + vetoed + " | Restam: " + slot.maps.join(", "));
      ...
      if (slot.maps.length === 1) {
        ...
        toCapChannel(slot,
          "---------------------------------\n" +
          "[MAPA]  MAPA DEFINIDO: [b]" + slot.chosenMap + "[/b]\n" +
          "---------------------------------\n" +
          ">> " + nameOf(slot.sideChooser) + " escolhe o lado!\n" +
          "   " + P + "lado gr  →  [GR] Global Risk\n" +
          "   " + P + "lado bl  →  [BL] Black List\n" +
          "[TEMPO] 3 minutos para escolher."
        );
        ...
        slot.vetoTimer = setTimeout(function() {
          ...
          toCapChannel(slot, "[TIMEOUT] Tempo esgotado! Bot escolheu: " + (rSide === "gr" ? "[GR] Global Risk" : "[BL] Black List"));
    ...
    function processSide(slot, chooserUid, side) {
      ...
      toCapChannel(slot,
        "[OK] Lado escolhido! Criando canais dos times...\n" +
        "[GR] Global Risk: " + nameOf(slot.grCap) + "\n" +
        "[BL] Black List: " + nameOf(slot.blCap)
      );
      ...
      var grChId = createChannel("[GR] Global Risk " + tag);
      if (!grChId) {
        broadcast("[AVISO] [" + tag + "] Falha ao criar canal Global Risk! Verifique permissões do bot.");
        return;
      }
      slot.grChId = grChId;
      ...
      setTimeout(function() {
        ...
        setTimeout(function() {
          var blChId = createChannel("[BL] Black List " + tag);
          if (!blChId) {
            broadcast("[AVISO] [" + tag + "] Falha ao criar canal Black List! Verifique permissões do bot.");
            return;
          }
          slot.blChId = blChId;
          ...
          setTimeout(function() {
            ...
            broadcast(
              "---------------------------------\n" +
              "[INICIO]  PARTIDA DEFINIDA  —  " + tag + "\n" +
              "[MAPA]  Mapa     : " + slot.chosenMap + "\n" +
              "[SRV]  Servidor : " + SERVER + "\n" +
              "[SENHA]  Senha    : " + slot.password + "\n" +
              "---------------------------------\n" +
              "[GR] Global Risk  —  Cap: " + nameOf(slot.grCap) + "\n" +
              "   " + slot.grTeam.map(nameOf).join(", ") + "\n" +
              "[BL] Black List   —  Cap: " + nameOf(slot.blCap) + "\n" +
            ...
            logMatch(
              "[" + tag + "] PARTIDA DEFINIDA | Mapa: " + slot.chosenMap +
              " | [GR](Cap:" + nameOf(slot.grCap) + "): " + slot.grTeam.map(nameOf).join(", ") +
              " | [BL](Cap:" + nameOf(slot.blCap) + "): " + slot.blTeam.map(nameOf).join(", ")
            );

            pokeClient(slot.grCap, "[CAP] Você é CAP da [GR] Global Risk em " + tag + "! Use " + P + "grwin para reportar vitória.");
            pokeClient(slot.blCap, "[CAP] Você é CAP da [BL] Black List em " + tag + "! Use " + P + "blwin para reportar vitória.");
    ...
    function registerVote(slot, voterUid, winner) {
      ...
      var tag       = "FILA#" + slot.matchNum;
      var teamLabel = isBL ? "[BL] Black List" : "[GR] Global Risk";
      var winLabel  = winner === "bl" ? "[BL] Black List" : "[GR] Global Risk";

      broadcast("[VOTO]  [" + tag + "] Cap da " + teamLabel + " (" + nameOf(voterUid) + ") reportou: " + winLabel + " venceu!");
      logMatch("[" + tag + "] VOTO | Cap " + teamLabel + " (" + nameOf(voterUid) + ") votou: " + winLabel);
      ...
      if (slot.blVote !== null && slot.grVote !== null) {
        if (slot.blVote === slot.grVote) {
          ...
          broadcast(
            "[AVISO]  [" + tag + "] Capitães divergiram!\n" +
            "   Cap BL votou: " + (slot.blVote === "bl" ? "[BL] Black List" : "[GR] Global Risk") + "\n" +
            "   Cap GR votou: " + (slot.grVote === "bl" ? "[BL] Black List" : "[GR] Global Risk") + "\n" +
            "[URGENTE] Admin necessário. Use " + P + "mix forçar <N> bl|gr"
          );
          logMatch("[" + tag + "] DISPUTA | BL votou: " + (slot.blVote === "bl" ? "BL" : "GR") + " | GR votou: " + (slot.grVote === "bl" ? "BL" : "GR") + " | Admin necessário");
          logAdmin("[" + tag + "] DISPUTA DE RESULTADO | BL Cap: " + nameOf(slot.blCap) + " votou " + slot.blVote + " | GR Cap: " + nameOf(slot.grCap) + " votou " + slot.grVote);
    ...
    function finishMatch(slot, winner, forced) {
      ...
      var tag    = "FILA#" + slot.matchNum;
      var wLabel = winner === "bl" ? "[BL] Black List" : "[GR] Global Risk";

      broadcast(
        "---------------------------------\n" +
        "[RESULT]  RESULTADO  —  " + tag + "\n" +
        "   Vencedor: " + wLabel + (forced ? " (admin)" : "") + "\n" +
        "---------------------------------"
      );

      logMatch("[" + tag + "] RESULTADO | Vencedor: " + wLabel + (forced ? " [FORÇADO POR ADMIN]" : "") + " | Mapa: " + (slot.chosenMap || "?") +
        " | [GR]: " + slot.grTeam.map(nameOf).join(", ") +
        " | [BL]: " + slot.blTeam.map(nameOf).join(", "));
    ...
    function cancelSlot(slot, adminName) {
      ...
      broadcast("[BAN] [ADMIN] " + adminName + " cancelou " + tag + ". Todos voltaram ao lobby.");
      logAdmin("[CANCELAR] " + adminName + " cancelou " + tag + ".");
      resetSlot(slot);
    }
    ...
    function cancelQueue(adminName) {
      ...
      queue.length = 0;
      broadcast("[BAN] [ADMIN] " + adminName + " limpou a fila de espera.");
      updateFilaName();
    }
    ...
    event.on("chat", function(ev) {
      ...
      if (cmd === P + "x5") {
        if (isBanned(uid)) {
          var expBan = formatExpiry(banExpiry(uid));
          var rsnBan = banReason(uid);
          client.chat("[BAN] Voce esta banido da fila.\nExpira: " + expBan + "\nMotivo: " + rsnBan);
          return;
        }
        if (slotOfMatch(uid)) {
          client.chat("[BAN] Você já está em uma partida ativa! Aguarde terminar.");
          return;
        }
        if (queue.indexOf(uid) !== -1) {
          client.chat("[AVISO] Você já está na fila. Use " + P + "sair para sair.");
          return;
        }
        ...
        if (config.filaChannelId) {
          ...
          if (cch && cch.id() !== config.filaChannelId) {
            ...
            client.chat("[X] Vá para o canal [b]" + (fch ? fch.name() : "Fila de Espera") + "[/b] para entrar na fila!");
            return;
          }
        }
        ...
        queue.push(uid);
        addFilaGroup(uid);
        // Adiciona [NA FILA] ao apelido visivel
        try {
          var curName = client.name();
          if (curName.indexOf("[NA FILA]") === -1) {
            client.setNickname("[NA FILA] " + curName);
          }
        } catch(e) {}
        toFila(null, "[OK] [" + client.name() + "] entrou! (" + queue.length + "/" + NEEDED + ")");
        updateFilaName();

        if (queue.length >= NEEDED) {
          toFila(null, "[GO] Fila completa! Iniciando mix...");
          tryStartMatch();
        }
        return;
      }
      ...
      if (cmd === P + "sair") {
        ...
        if (idx === -1) { client.chat("[AVISO] Você não está na fila de espera."); return; }
        queue.splice(idx, 1);
        removeFilaGroup(uid);
        try {
          var n2 = client.name();
          if (n2.indexOf("[NA FILA] ") !== -1) {
            client.setNickname(n2.replace("[NA FILA] ", ""));
          }
        } catch(e) {}
        toFila(null, "[X] [" + client.name() + "] saiu da fila. (" + queue.length + "/" + NEEDED + ")");
        updateFilaName();
        return;
      }
      ...
      if (cmd === P + "fila" || cmd === P + "filas") {
        var statusMap = { veto:"[FIL] Veto", side:"[FIL] Escolha de lado", playing:"[BL] Em jogo", disputed:"* Disputa" };
        var lines = ["[LISTA] Fila de espera: " + queue.length + "/" + NEEDED + " jogadores"];
        if (queue.length > 0) {
          lines.push("   " + queue.map(nameOf).join(", "));
        }
        lines.push("[MIX] Partidas ativas: " + matches.length);
      ...
      if (cmd === P + "vetar") {
        ...
        if (!sm || sm.status !== "veto") { client.chat("[AVISO] Não há veto ativo para você agora."); return; }
        if (!isCaptainOf(sm, uid)) { client.chat("[AVISO] Você não é capitão nesta partida."); return; }
        var currentCap = sm.vetoTurn === "cap1" ? sm.cap1 : sm.cap2;
        if (uid !== currentCap) {
          client.chat("[AVISO] Não é sua vez! Aguarde " + nameOf(currentCap) + " vetar.");
          return;
        }
        var idx = parseInt(parts[1]) - 1;
        if (isNaN(idx) || idx < 0 || idx >= sm.maps.length) {
          client.chat("[X] Número inválido. Escolha entre 1 e " + sm.maps.length + ":\n" + mapListText(sm.maps));
          return;
        }
      ...
      if (cmd === P + "lado") {
        ...
        if (!sm2 || sm2.status !== "side") { client.chat("[AVISO] Não há escolha de lado ativa para você agora."); return; }
        if (uid !== sm2.sideChooser) {
          client.chat("[AVISO] Não é você que escolhe o lado. É " + nameOf(sm2.sideChooser) + ".");
          return;
        }
        var side = (parts[1] || "").toLowerCase();
        if (side !== "gr" && side !== "bl") {
          client.chat("[X] Use: " + P + "lado gr  ([GR] Global Risk)  ou  " + P + "lado bl  ([BL] Black List)");
          return;
        }
      ...
      if (cmd === P + "blwin" || cmd === P + "grwin") {
        ...
        if (!sm3 || (sm3.status !== "playing" && sm3.status !== "disputed")) {
          client.chat("[AVISO] Não há partida ativa para reportar resultado.");
          return;
        }
        if (uid !== sm3.blCap && uid !== sm3.grCap) {
          client.chat("[AVISO] Você não é capitão desta partida.");
          return;
        }
        if (sm3.status === "disputed" && !isAdmin(client)) {
          client.chat("[AVISO] Resultado em disputa. Aguarde um administrador.");
          return;
        }
      ...
      if (cmd === P + "admin") {
        ...
        if (sm4) {
          ...
          if (sm4.adminCalled) {
            client.chat("[TEMPO] Um admin já foi chamado para esta partida. Aguarde.");
            return;
          }
        ...
        var msg = "[URGENTE] " + client.name() + " chamou admin (" + contexto + "): " + motivo;
        broadcast(msg);
        pokeAdmins(client.name() + " chamou admin em " + contexto + ". Motivo: " + motivo);
        client.chat("[OK] Todos os admins foram notificados!");
        return;
      }
      ...
      if (cmd === P + "uid") {
        if (!isAdmin(client)) { client.chat("[X] Permissão negada."); return; }
        var allC = backend.getClients();
        var lines2 = ["[LISTA] UIDs online (" + allC.length + "):"];
      ...
      if (cmd === P + "registrar") {
        if (!PUG_GID) {
          client.chat("[AVISO] Cargo PUG/MIX não configurado. Peça ao admin para configurar o pugGroupId.");
          return;
        }
        ...
        if (jaTemPug) {
          client.chat("[OK] Você já possui o cargo PUG/MIX!");
          return;
        }
        addPugGroup(uid);
        client.chat("[OK] Cargo PUG/MIX atribuído! Agora você receberá notificações de mix.");
        broadcast("[MIX] [" + client.name() + "] se registrou para o PUG/MIX!");
        return;
      }
      ...
      if (cmd === P + "desregistrar") {
        if (!PUG_GID) {
          client.chat("[AVISO] Cargo PUG/MIX não configurado.");
          return;
        }
        removePugGroup(uid);
        client.chat("[OK] Cargo PUG/MIX removido. Use " + P + "registrar para voltar.");
        return;
      }
      ...
      if (cmd === P + "mix") {
        ...
        if (sub === "help" || sub === "") {
          client.chat(
            ...
            P + "lado gr|bl           → (Cap) Escolhe lado (gr=[GR] Global Risk, bl=[BL] Black List)\n" +
            ...
            P + "mix resetar                  → [AVISO] Reseta TUDO à força (emergência)\n" +
            P + "mix limparfila               → Limpa fila de espera (mantém partidas)\n" +
            P + "mix ban <uid> [dur] [motivo] → Bane usuario (dur: 2h, 3d, 1m ou vazio=perm)\n" +
        ...
        if (sub === "ban") {
          var target = parts[2] || "";
          if (!target) {
            client.chat("[X] Use: !mix ban <uid> [duracao] [motivo]\nDuracao: 2h, 3d, 1m ou vazio = permanente\nEx: !mix ban <uid> 2h spam  |  !mix ban <uid> (permanente)");
            return;
          }
          var durStr  = parts[3] || "";
          var hasDur  = /^[0-9]+(h|d|m)$/i.test(durStr);
          var durMs   = hasDur ? parseBanDuration(durStr) : null;
          var reason  = hasDur ? parts.slice(4).join(" ") : parts.slice(3).join(" ");
          if (!reason) reason = "sem motivo";
          banPlayer(target, durMs, reason);
          var bc = getClient(target);
          var bn = bc ? bc.name() : target;
          var expLabel = formatExpiry(durMs ? Date.now() + durMs : null);
          client.chat("[OK] " + bn + " banido!\nExpira: " + expLabel + "\nMotivo: " + reason);
          broadcast("[BAN] [ADMIN] " + client.name() + " baniu " + bn + ". Expira: " + expLabel);
          logAdmin("[BAN] " + client.name() + " baniu " + bn + " (UID:" + target + ") | Expira: " + expLabel + " | Motivo: " + reason);
          ...
          if (bi !== -1) {
            ...
            try {
              var bc2 = getClient(target);
              if (bc2) {
                var bn2 = bc2.name();
                if (bn2.indexOf("[NA FILA] ") !== -1) bc2.setNickname(bn2.replace("[NA FILA] ", ""));
              }
            } catch(e) {}
            updateFilaName();
          }
        ...
        if (sub === "unban") {
          var target2 = parts[2] || "";
          if (!target2) { client.chat("[X] Use: " + P + "mix unban <uid>"); return; }
          if (unbanPlayer(target2)) {
            var bc3 = getClient(target2);
            client.chat("[OK] " + (bc3 ? bc3.name() : target2) + " desbanido!");
          } else { client.chat("[AVISO] Este usuário não está banido."); }
          return;
        }

        // !mix banlist — lista banidos
        if (sub === "banlist") {
          var bkeys = Object.keys(bannedList);
          // Filtra expirados
          bkeys = bkeys.filter(function(u){ return isBanned(u); });
          if (bkeys.length === 0) { client.chat("[LISTA] Nenhum usuario banido."); return; }
          var blines = ["[LISTA] Banidos (" + bkeys.length + "):"];
          bkeys.forEach(function(u, i) {
            var bc4 = getClient(u);
            var exp = formatExpiry(banExpiry(u));
            var rsn = banReason(u);
            blines.push("  " + (i+1) + ". " + (bc4 ? bc4.name() : "(offline)") + "\n     UID: " + u + "\n     Expira: " + exp + " | Motivo: " + rsn);
          });
        ...
        if (sub === "limparfila") {
          queue.forEach(function(u) {
            ...
            try {
              ...
              if (lc) {
                var ln = lc.name();
                if (ln.indexOf("[NA FILA] ") !== -1) lc.setNickname(ln.replace("[NA FILA] ", ""));
              }
            } catch(e) {}
          });
          ...
          queue.length = 0;
          updateFilaName();
          broadcast("[LIMPAR] [ADMIN] " + client.name() + " limpou a fila de espera. (" + removed + " jogador(es) removidos)");
          client.chat("[OK] Fila de espera limpa!");
          return;
        }

        // !mix status
        if (sub === "status") {
          var lines2 = ["[LISTA] [ADMIN] Status — Total: " + matchCounter + " | Fila: " + queue.length + "/" + NEEDED];
        ...
        if (sub === "cancelar") {
          ...
          if (arg === "todos") {
            ...
            client.chat("[OK] Todas as partidas e fila canceladas.");
            return;
          }
          if (arg === "fila") {
            cancelQueue(client.name());
          ...
          if (!isNaN(num) && num > 0) {
            ...
            if (target) { cancelSlot(target, client.name()); client.chat("[OK] FILA#" + num + " cancelada."); return; }
            client.chat("[X] Nenhuma partida ativa com número " + num + ".");
            return;
          }
          if (matches.length > 0) {
            cancelSlot(matches[0], client.name());
          } else {
            client.chat("[AVISO] Nenhuma partida ativa para cancelar. Use: " + P + "mix cancelar fila");
          }
          return;
        }
        ...
        if (sub === "resetar") {
          ...
          broadcast(
            "[RESET] [ADMIN] " + client.name() + " resetou todas as filas!\n" +
            "   " + totalMoved + " jogador(es) movidos ao lobby | " + totalChannels + " canal(is) deletado(s)."
          );
          logAdmin("[RESET TOTAL] " + client.name() + " resetou todas as filas. " + totalMoved + " jogadores movidos, " + totalChannels + " canais deletados.");
          client.chat("[OK] Reset completo! Todas as filas estão limpas e abertas.");
          return;
        }
        if (sub === "filateste") {
          var testSize = Math.max(2, Math.min(10, parseInt(parts[2]) || 2));

          // Pega SOMENTE quem esta no canal da fila (exceto bot)
          var filaCh2  = getChannel(config.filaChannelId);
          var inFila2  = filaCh2 ? filaCh2.getClients().filter(function(c){ return !c.isSelf(); }) : [];

          if (inFila2.length < 2) {
            client.chat("[X] Precisa de pelo menos 2 jogadores no canal da fila. Agora: " + inFila2.length);
            return;
          }

          // Prioriza: quem chamou primeiro, depois admins, depois resto
          var callerIn2 = inFila2.some(function(c){ return c.uid() === uid; });
          var others2   = inFila2.filter(function(c){ return c.uid() !== uid; });
          var admins2   = others2.filter(function(c){ return isAdmin(c); });
          var normals2  = others2.filter(function(c){ return !isAdmin(c); });
          var base2     = callerIn2 ? [client] : [];
          var ordered2  = base2.concat(admins2).concat(normals2).slice(0, testSize);

          if (ordered2.length < 2) {
            client.chat("[X] Jogadores insuficientes no canal da fila (" + ordered2.length + ").");
            return;
          }

          var snapTest = ordered2.map(function(c){ return c.uid(); });
          broadcast("[TESTE] " + client.name() + " iniciou fila de teste com " + ordered2.length + " jogador(es): " + ordered2.map(function(c){ return c.name(); }).join(", "));

          var oldNeeded2 = NEEDED;
          NEEDED = ordered2.length;
          // Passa diretamente para tryStartMatch simulando a fila
          var prevQueue = queue.slice();
          queue.length = 0;
          snapTest.forEach(function(u){ queue.push(u); });
          tryStartMatch();
          // Restaura quem estava na fila antes (exceto os do teste)
          prevQueue.forEach(function(u){
            if (snapTest.indexOf(u) === -1 && queue.indexOf(u) === -1) queue.push(u);
          });
          NEEDED = oldNeeded2;
          updateFilaName();
          return;
        ...
        if (sub === "forçar" || sub === "forcar") {
          ...
          if (isNaN(forceNum) || (forceWinner !== "bl" && forceWinner !== "gr")) {
            client.chat("[X] Use: " + P + "mix forçar <número da fila> bl|gr\n   Ex: " + P + "mix forçar 3 bl");
            return;
          }
          var forceSlot = slotByMatchNum(forceNum);
          if (!forceSlot || (forceSlot.status !== "disputed" && forceSlot.status !== "playing")) {
            client.chat("[X] Sem partida ativa com número " + forceNum + " para forçar.");
            return;
          }
          broadcast("[ADMIN] [ADMIN] " + client.name() + " forçou resultado da FILA#" + forceNum + ".");
          logAdmin("[FORÇAR] " + client.name() + " forçou resultado da FILA#" + forceNum + " para " + (forceWinner === "bl" ? "[BL] Black List" : "[GR] Global Risk") + ".");
        ...
        if (sub === "vetar") {
          ...
          if (isNaN(fvNum)) {
            client.chat("[X] Use: " + P + "mix vetar <fila> <num_mapa>  |  Ex: " + P + "mix vetar 3 2");
            return;
          }
          var fvSlot = slotByMatchNum(fvNum);
          if (!fvSlot || fvSlot.status !== "veto") {
            client.chat("[X] FILA#" + fvNum + " não está em fase de veto. Use !fila para ver o status.");
            return;
          }
          if (isNaN(fvMap) || fvMap < 0 || fvMap >= fvSlot.maps.length) {
            client.chat("[X] Mapa inválido. Disponíveis em FILA#" + fvNum + ":\n" + mapListText(fvSlot.maps));
            return;
          }
          var currentCapUid = fvSlot.vetoTurn === "cap1" ? fvSlot.cap1 : fvSlot.cap2;
          broadcast("[ADMIN] [ADMIN] " + client.name() + " forçou veto do mapa " + fvSlot.maps[fvMap] + " em FILA#" + fvNum + " (pelo cap ausente " + nameOf(currentCapUid) + ").");
        ...
        if (sub === "lado") {
          ...
          if (isNaN(flNum) || (flSide !== "gr" && flSide !== "bl")) {
            client.chat("[X] Use: " + P + "mix lado <fila> gr|bl  |  Ex: " + P + "mix lado 3 gr");
            return;
          }
          var flSlot = slotByMatchNum(flNum);
          if (!flSlot || flSlot.status !== "side") {
            client.chat("[X] FILA#" + flNum + " não está em fase de escolha de lado.");
            return;
          }
          broadcast("[ADMIN] [ADMIN] " + client.name() + " forçou lado " + (flSide === "gr" ? "[GR] Global Risk" : "[BL] Black List") + " em FILA#" + flNum + " (pelo cap ausente " + nameOf(flSlot.sideChooser) + ").");
        ...
        if (sub === 'vitoria') {
          ...
          if (isNaN(vitNum) || (vitWinner !== 'bl' && vitWinner !== 'gr')) {
            client.chat('[X] Use: ' + P + 'mix vitoria <numero da fila> bl|gr  |  Ex: ' + P + 'mix vitoria 3 bl');
            return;
          }
          var vitSlot = slotByMatchNum(vitNum);
          if (!vitSlot || (vitSlot.status !== 'playing' && vitSlot.status !== 'disputed')) {
            client.chat('[X] Sem partida ativa com número ' + vitNum + '. Use !mix forçar para partidas já encerradas que travaram.');
            return;
          }
          var vitLabel = vitWinner === 'bl' ? '[BL] Black List' : '[GR] Global Risk';
          broadcast('[ADMIN] [ADMIN] ' + client.name() + ' corrigiu vitória da FILA#' + vitNum + ' para ' + vitLabel + '.');
          finishMatch(vitSlot, vitWinner, true);
          return;
        }

        // !mix limparranking — zera ranking e salva histórico (só admins)
        if (sub === "limparranking") {
          clearRanking(client.name(), false);
          client.chat("[OK] Ranking zerado! Histórico salvo antes da limpeza.");
          return;
        }
        ...
        if (sub === 'pts') {
          ...
          if (!ptsUid || isNaN(ptsVal)) {
            client.chat('[X] Use: ' + P + 'mix pts <uid> <pontos>  |  Ex: ' + P + 'mix pts ABC123= 15  | Dica: use !mix ranking para ver UIDs');
            return;
          }
          var r = getRanking();
          ...
          saveRanking(r);
          updateRankingChannel();
          client.chat('[OK] Pontos de ' + r[ptsUid].name + ' alterados: ' + oldPts + ' → ' + ptsVal);
        ...
        if (sub === 'ranking') {
          ...
          var rlines = ['[LISTA] [ADMIN] Ranking completo:'];
        ...
        if (sub === "admin") {
          ...
          var target = parts[3] || "";
          if (action === "listar") {
            var alines = ["[LISTA] Admins (" + adminUids.length + "):"];
          ...
          if (action === "add") {
            if (!target) { client.chat("[X] " + P + "mix admin add <uid>"); return; }
            if (addAdminUid(target)) {
              var ac2 = getClient(target);
              var aname2 = ac2 ? ac2.name() : target;
              client.chat("[OK] " + aname2 + " adicionado como admin!");
              broadcast("[SENHA] " + client.name() + " adicionou " + aname2 + " como admin.");
              if (ac2) ac2.poke("[OK] Você foi adicionado como admin do Mix por " + client.name() + "!");
            } else { client.chat("[AVISO] UID já está na lista."); }
            return;
          }
          if (action === "remove") {
            if (!target) { client.chat("[X] " + P + "mix admin remove <uid>"); return; }
            if (removeAdminUid(target)) {
              var ac3 = getClient(target);
              client.chat("[OK] " + (ac3 ? ac3.name() : target) + " removido dos admins.");
            } else { client.chat("[AVISO] UID não encontrada."); }
            return;
          }
          client.chat("[X] " + P + "mix admin add <uid> | remove <uid> | listar");
          return;
        }
        ...
        if (sub === "pai") {
          ...
          var lines3 = ["[LISTA] Info dos canais configurados:"];
          if (f1) {
        ...
        if (sub === "canais") {
          var chs = backend.getChannels();
          var lines = ["[LISTA] Canais do servidor (use os IDs no SinusBot):"];
        ...
        if (sub === "update") {
          ...
          client.chat(
            "[AVISO] Auto-update não suportado nesta versão do SinusBot.\n" +
            "Rode na VPS:\ncurl -o " + PATH + " " + URL + " && systemctl restart sinusbot"
          );
          return;
        }

        client.chat("[X] Subcomando inválido. Use " + P + "mix help");
        return;
      }
    });
    ...
    event.on("clientDisconnect", function(ev) {
      ...
      if (qidx !== -1) {
        queue.splice(qidx, 1);
        removeFilaGroup(uid);
        toFila(null, "[AVISO] [" + name + "] desconectou e saiu da fila. (" + queue.length + "/" + NEEDED + ")");
        updateFilaName();
      }
      ...
      matches.forEach(function(s) {
        if ((s.status === "veto" || s.status === "side") && (uid === s.cap1 || uid === s.cap2)) {
          broadcast("[URGENTE] Capitão " + name + " desconectou durante veto/lado em FILA#" + s.matchNum + "!");
        ...
        if ((s.status === "playing" || s.status === "disputed") && (uid === s.blCap || uid === s.grCap)) {
          var role = uid === s.blCap ? "[BL] Black List" : "[GR] Global Risk";
          broadcast("[URGENTE] Capitão da " + role + " (" + name + ") desconectou em FILA#" + s.matchNum + "!");
    ...
    updateFilaName();
    engine.log("[OK] Mix Shuffle v9 carregado! Prefix=" + P + " | !x5 para entrar | Partidas=" + matchCounter);
  }
);
