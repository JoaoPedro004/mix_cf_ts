...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    event.on("chat", function(ev) {
      ...
      if (cmd === P + "x5") {
        if (isBanned(uid)) {
          client.chat("[BAN] Você está banido da fila. Entre em contato com um administrador.");
          return;
        }
        if (slotOfMatch(uid)) {
      ...
      if (cmd === P + "mix") {
        ...
        if (sub === "ban") {
          var target = parts[2] || "";
          if (!target) { client.chat("[X] Use: " + P + "mix ban <uid>"); return; }
          if (banPlayer(target)) {
            var bc = getClient(target);
            var bn = bc ? bc.name() : target;
            client.chat("[OK] " + bn + " banido da fila!");
            broadcast("[BAN] [ADMIN] " + client.name() + " baniu " + bn + " da fila.");
          ...
          } else { client.chat("[AVISO] Este usuário já está banido."); }
          return;
        }
        ...
        if (sub === "banlist") {
          if (bannedUids.length === 0) { client.chat("[LISTA] Nenhum usuário banido."); return; }
          var blines = ["[LISTA] Banidos (" + bannedUids.length + "):"];
        ...
        if (sub === "filateste") {
          ...
          if (!testSlot) {
            client.chat("[X] Nenhuma fila aberta para teste. Use " + P + "mix cancelar primeiro.");
            return;
          }
          ...
          if (inFila.length < 2) {
            client.chat(
              "[X] Precisa de pelo menos 2 jogadores no canal da fila para o teste.\n" +
          ...
          if (callerInFila) {
            ...
            client.chat("[AVISO] Você não está no canal da fila, mas o teste será iniciado com quem está lá.");
            ordered2 = admins2.concat(normals2);
          }
          ordered2 = ordered2.slice(0, testSize);

          if (ordered2.length < 2) {
            client.chat("[X] Jogadores insuficientes no canal da fila (" + ordered2.length + " encontrado(s)).");
            return;
          }
          ...
          broadcast(
            "[TESTE] [TESTE] " + client.name() + " iniciou teste na Fila " + (testSlot.slotIndex+1) +
