...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    function processSide(slot, chooserUid, side) {
      ...
      // Avisa no canal dos capitães antes de deletar
      toCapChannel(slot,
      ...
      if (!grChId || !blChId) {
        broadcast("⚠️ [" + tag + "] Erro ao criar canais dos times! Verifique permissões.");
        if (grChId) deleteChannel(grChId);
        if (blChId) deleteChannel(blChId);
      } else {
        // Move todos do time GR para o canal Global Risk
        slot.grTeam.forEach(function(u) {
          var c = getClient(u);
          if (c) c.moveTo(grChId);
        });
        // Move todos do time BL para o canal Black List
      ...
      // Anuncia resultado no canal principal
      broadcast(
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" +
      ...
      // Agora que o veto/lado terminou, desbloqueia fila 2
      updateAllFilaNames();
      if (slot.slotIndex === 0) {
