...
registerPlugin(
  {
    ...
    vars: [
      ...
      },
      {
        ...
        title: "Canal PAI dos Times — onde Time 1 e Time 2 serão criados (pode ser o mesmo que o LOBBY)",
        type: "channel",
      },
      {
        name: "serverName",
  ...
  function (sinusbot, config) {
    ...
    function createMatchChannel(name, afterChannelId) {
      try {
        // Canais dos times são criados como subcanais do canal PAI configurado
        var parentId = config.parentChannelId || config.lobbyChannelId || "0";
        var props = { name: name, parent: parentId, permanent: false };

        if (afterChannelId) {
          props.order = afterChannelId;
        } else if (config.stopBeforeChannelId) {
          var siblings = backend.getChannels().filter(function(c) {
            return c.parent() && c.parent().id() === parentId;
          });
          var stopIdx = -1;
          for (var i = 0; i < siblings.length; i++)
            if (siblings[i].id() === config.stopBeforeChannelId) { stopIdx = i; break; }
    ...
    function updateChannelName() {
      ...
      try {
        ...
        if (mainQueue.status === "open") {
          if (count === 0) {
            // Vermelho — vazio
            newName = "[cspacer]🔴 Fila de espera [ 0/" + NEEDED + " ]";
          } else if (count <= 4) {
            // Vermelho
            newName = "[cspacer]🔴 Fila de espera [ " + count + "/" + NEEDED + " ]";
          } else if (count <= 7) {
            // Amarelo
            newName = "[cspacer]🟡 Fila de espera [ " + count + "/" + NEEDED + " ]";
          ...
          newName = "[cspacer]⏳ Fila de espera [ aguardando... ]";
        }
        ch.setName(newName);
      } catch(e) {}
    }
    ...
    function startMatch(q) {
      ...
      // Cria canais "Time 1" e "Time 2" — Time 2 vem logo após Time 1
      var t1ChId = createMatchChannel("Time 1 — " + tag, null);
      var t2ChId = createMatchChannel("Time 2 — " + tag, t1ChId);
    ...
    function startVetoRound(q) {
      ...
      var otherUid   = q.vetoTurn === "cap1" ? q.cap2 : q.cap1;
      var capClient  = getClientByUid(capUid);
      var otherClient = getClientByUid(otherUid);
      ...
      // Manda lista de mapas para os dois capitães no PV
      if (capClient) {
        capClient.chat(vetoMsg + "\n👉 É a SUA VEZ de vetar! Use " + P + "vetar <número>");
        capClient.poke("👉 Sua vez de vetar em " + tag + "! Veja o PV.");
      }
      if (otherClient) {
        otherClient.chat(vetoMsg + "\n⏳ Aguarde " + capName + " vetar.");
      }
      ...
      q.vetoTimer = setTimeout(function() {
        ...
        var autoVetoed = q.maps[randomIdx];
        if (capClient) capClient.chat("⏰ Você demorou! Bot vetou automaticamente: " + autoVetoed);
        if (otherClient) otherClient.chat("⏰ " + capName + " demorou! Bot vetou automaticamente: " + autoVetoed);
        broadcast("⏰ [" + tag + "] " + capName + " demorou! Bot vetou: " + autoVetoed);
    ...
    function processVeto(q, voterUid, mapIdx) {
      ...
      var tag         = "FILA#" + q.matchNum;
      var cap1Client  = getClientByUid(q.cap1);
      var cap2Client  = getClientByUid(q.cap2);

      q.vetoOrder.push(voterUid);

      var vetoNotice = "❌ " + nameOf(voterUid) + " vetou: [b]" + vetoed + "[/b] | Restam " + q.maps.length + " mapa(s)";
      if (cap1Client) cap1Client.chat(vetoNotice);
      if (cap2Client) cap2Client.chat(vetoNotice);
      ...
      if (q.maps.length === 1) {
        ...
        var sideChooserClient = getClientByUid(q.sideChooser);
        var otherClient       = getClientByUid(lastVetoerIsCap1 ? q.cap1 : q.cap2);

        var sideMsg =
          "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" +
          "🗺️  MAPA DEFINIDO: [b]" + q.chosenMap + "[/b]\n" +
          "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" +
          "⏳ Timeout: 3 minutos";

        if (sideChooserClient) {
          sideChooserClient.chat(sideMsg + "\n👉 É a SUA VEZ de escolher o lado!\n" + P + "lado gr  →  Global Risk\n" + P + "lado tr  →  Terroristas");
          sideChooserClient.poke("👉 Escolha o lado em " + tag + "! Veja o PV.");
        }
        if (otherClient) {
          otherClient.chat(sideMsg + "\n⏳ Aguarde " + nameOf(q.sideChooser) + " escolher o lado.");
        }

        // Anuncia no chat principal também
        broadcast("🗺️  [" + tag + "] Mapa definido: " + q.chosenMap + " | " + nameOf(q.sideChooser) + " escolhe o lado no PV.");

        // Timer para escolha de lado
        q.vetoTimer = setTimeout(function() {
          if (q.status !== "side") return;
          var randomSide = Math.random() < 0.5 ? "gr" : "tr";
          var sideLabel  = randomSide === "gr" ? "Global Risk" : "Terroristas";
          if (sideChooserClient) sideChooserClient.chat("⏰ Você demorou! Bot escolheu: " + sideLabel);
          broadcast("⏰ [" + tag + "] " + nameOf(q.sideChooser) + " demorou! Bot escolheu lado: " + sideLabel);
      ...
      if (cmd === P + "mix") {
        ...
        if (sub === "help" || sub === "") {
          client.chat(
            ...
            P + "mix filateste <n>           → Inicia fila de teste com N jogadores (mín. 2)\n" +
        ...
        // !mix filateste <número de jogadores>
        if (sub === "filateste") {
          var testSize = parseInt(parts[2]) || 2;
          if (testSize < 2) testSize = 2;
          if (testSize > 10) testSize = 10;

          if (mainQueue.status !== "open") {
            client.chat("❌ A fila não está aberta. Use !mix reset para liberar.");
            return;
          }

          // Pega jogadores online excluindo o bot
          var allOnline = backend.getClients().filter(function(c) { return !c.isSelf(); });

          if (allOnline.length < 2) {
            client.chat("❌ Precisa de pelo menos 2 jogadores online.");
            return;
          }

          // Separa: admin que chamou (sempre primeiro = cap1), outros admins, depois o resto
          var caller = client;
          var otherAdmins = allOnline.filter(function(c) {
            return c.uid() !== caller.uid() && isAdmin(c);
          });
          var normalPlayers = allOnline.filter(function(c) {
            return c.uid() !== caller.uid() && !isAdmin(c);
          });

          // Monta lista: caller primeiro, depois outros admins, depois normais
          var prioritized = [caller].concat(otherAdmins).concat(normalPlayers);
          var testPlayers  = prioritized.slice(0, testSize);

          if (testPlayers.length < 2) {
            client.chat("❌ Não há jogadores suficientes online (" + testPlayers.length + " encontrado(s)).");
            return;
          }

          mainQueue.players = testPlayers.map(function(c) { return c.uid(); });
          mainQueue.status  = "closed";

          broadcast(
            "🧪 [TESTE] " + client.name() + " iniciou uma fila de teste com " +
            testPlayers.length + " jogador(es): " +
            testPlayers.map(function(c){ return c.name(); }).join(", ")
          );

          var oldNEEDED = NEEDED;
          NEEDED = testPlayers.length;
          startMatch(mainQueue);
          NEEDED = oldNEEDED;
          return;
        }
