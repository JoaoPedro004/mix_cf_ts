  ...
  function (sinusbot, config) {
    ...
    function addFilaGroup(uid) {
      ...
      try {
        var c = getClient(uid);
        if (!c) return;
        // Tenta método 1: backend direto
        if (typeof backend.addServerGroupToClient === "function") {
          backend.addServerGroupToClient(c, FILA_GID);
        }
        // Tenta método 2: via ServerQuery
        else if (typeof backend.exec === "function") {
          backend.exec("servergroupaddclient sgid=" + FILA_GID + " cldbid=" + c.databaseId());
        }
        // Tenta método 3: via client object
        else if (typeof c.serverGroupIds === "function") {
          var groups = c.serverGroupIds();
          if (groups.indexOf(String(FILA_GID)) === -1) {
            c.addToServerGroup(FILA_GID);
          }
        }
      } catch(e) {
        engine.log("[Mix] Erro ao adicionar cargo FILA para " + uid + ": " + e);
      }
    }
    ...
    function removeFilaGroup(uid) {
      ...
      try {
        ...
        if (!c) return;
        // Tenta método 1: backend direto
        if (typeof backend.removeServerGroupFromClient === "function") {
          backend.removeServerGroupFromClient(c, FILA_GID);
        }
        // Tenta método 2: via ServerQuery
        else if (typeof backend.exec === "function") {
          backend.exec("servergroupdelclient sgid=" + FILA_GID + " cldbid=" + c.databaseId());
        }
        // Tenta método 3: via client object
        else if (typeof c.removeFromServerGroup === "function") {
          c.removeFromServerGroup(FILA_GID);
        }
      } catch(e) {
        engine.log("[Mix] Erro ao remover cargo FILA de " + uid + ": " + e);
      }
    }
    ...
    function startMatch(slot) {
      ...
      // Embaralha e divide os times — garante pelo menos 1 por time
      var sh   = shuffle(players);
      var half = Math.max(1, Math.floor(sh.length / 2));
      var t1p  = sh.slice(0, half);
      var t2p  = sh.slice(half);

      // Se t2p ficou vazio (caso com exatamente 1 jogador), move o último de t1p para t2p
      if (t2p.length === 0) {
        t2p.push(t1p.pop());
      }
      ...
      // Remove cargo FILA de todos ao iniciar a partida
      slot.players.forEach(function(u){ removeFilaGroup(u); });
      slot.players = [];
      ...
      // Arrasta apenas os dois capitães para o canal de veto
