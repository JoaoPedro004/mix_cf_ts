...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    function loadFilaPrefix() {
      var ch = getChannel(config.filaChannelId);
      if (!ch) return;
      var name = ch.name();
      // Extrai prefixo tipo [cspacer], [cspacer0], [rspacer] etc
      var match = name.match(/^(\[.+?\])/);
      filaNamePrefix = match ? match[1] : "";
      engine.log("[Mix] Prefixo do canal fila: '" + filaNamePrefix + "'");
    }
    ...
    function updateFilaName() {
      ...
      try {
        ...
        var emoji = n <= 4 ? "🔴" : n <= 7 ? "🟡" : "🟢";
        var check = n >= NEEDED ? " ✅" : "";
        ch.setName(filaNamePrefix + emoji + " Fila de espera [ " + n + "/" + NEEDED + check + " ]");
      } catch(e) {}
    }
    ...
    // Lê o prefixo do canal quando o bot conectar ao servidor
    event.on("connect", function() {
      setTimeout(function() {
        loadFilaPrefix();
        updateFilaName();
        engine.log("[Mix] Bot conectado, fila inicializada.");
      }, 2000);
    });
