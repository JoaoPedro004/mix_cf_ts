...
registerPlugin(
  ...
  function (sinusbot, config) {
      ...
      if (cmd === P + "mix") {
        ...
        if (sub === "update") {
          ...
          http.simpleGet(GITHUB_URL, function(err, data) {
            if (err || !data) { client.chat("❌ Erro ao baixar: " + (err || "sem resposta")); return; }
            try {
              var fs = require("fs");
              fs.writeFile(SCRIPT_PATH, data, function(werr) {
                if (werr) { client.chat("❌ Erro ao salvar: " + werr); return; }
                client.chat("✅ Atualizado! Reiniciando em 3s...");
                broadcast("🔄 [ADMIN] " + client.name() + " atualizou o bot. Reiniciando...");
                setTimeout(function() { engine.restart(); }, 3000);
              });
            } catch(e) { client.chat("❌ Erro: " + e); }
          });
          return;
        }
    ...
    setInterval(function() {
      updateChannelName();
      if (mainQueue.status !== "open" || mainQueue.players.length === 0) return;
      var names = mainQueue.players.map(function(u){ return nameOf(u); });
      broadcastToChannel(
        config.filaChannelId,
        "📋 Fila atual: " + mainQueue.players.length + "/" + NEEDED + "\n" +
        "   " + names.join(", ") + "\n" +
