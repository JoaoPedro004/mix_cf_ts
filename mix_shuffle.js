...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    function createChannel(name, parentId) {
      try {
        // parentId pode ser passado diretamente; fallback para config
        var pid = parentId || config.parentChannelId || "0";
    ...
    function startMatch(slot) {
      ...
      // Cria canal dos capitães como filho do canal da fila do slot
      var filaParent = slot.slotIndex === 0 ? config.filaChannelId : (config.fila2ChannelId || config.filaChannelId);
      var capChId = createChannel("⚔️ CAPITÃES VETANDO " + tag, filaParent);
      if (!capChId) {
    ...
    function processSide(slot, chooserUid, side) {
      ...
      // Deleta canal de capitães após 1s
      var capChToDelete = slot.capChId;
      slot.capChId = null;
      setTimeout(function() { deleteChannel(capChToDelete); }, 1000);

      // Cria GR primeiro, depois BL com delay de 1s entre eles,
      // e só move os jogadores após ambos existirem (mais 500ms)
      var grChId = createChannel("🔵 Global Risk " + tag);
      slot.grChId = grChId;

      if (!grChId) {
        broadcast("⚠️ [" + tag + "] Falha ao criar canal Global Risk! Verifique permissões.");
        return;
      }
      ...
      setTimeout(function() {
        ...
        if (!blChId) {
          broadcast("⚠️ [" + tag + "] Falha ao criar canal Black List! Verifique permissões.");
          return;
        }

        // Ambos criados — move os jogadores após mais 500ms
        setTimeout(function() {
        ...
        // Desbloqueia fila 2 agora que o veto/lado terminou
        updateAllFilaNames();
        if (slot.slotIndex === 0) {
          toFila(slots[1], "🔓 Fila 2 desbloqueada! Use " + P + "entrar para o próximo mix.");
        }

      }, 1000); // 1s após criar o GR, cria o BL
    }
