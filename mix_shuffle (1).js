...
registerPlugin(
  ...
  function (sinusbot, config) {
    ...
    var adminUids = store.get("adminUids") || [];
    ...
    function startMatch(q) {
      ...
      updateChannelName();
      broadcast("✅ A fila está aberta novamente! Use " + P + "entrar para o próximo mix.");
    }
    ...
    event.on("chat", function (ev) {
      ...
      if (cmd === P + "entrar") {
        ...
        broadcast("✅ [" + client.name() + "] entrou na fila! (" + mainQueue.players.length + "/" + NEEDED + ")");
        updateChannelName();

        if (mainQueue.players.length >= NEEDED) {
          broadcast("🚀 Fila completa! Iniciando mix...");
      ...
      if (cmd === P + "sair") {
        ...
        broadcast("❌ [" + client.name() + "] saiu da fila. (" + found.players.length + "/" + NEEDED + ")");
        updateChannelName();
        return;
      }
    ...
    event.on("clientDisconnect", function (ev) {
      ...
      if (found) {
        found.players = found.players.filter(function(u){ return u !== uid; });
        broadcast("⚠️ [" + name + "] desconectou e saiu da fila. (" + found.players.length + "/" + NEEDED + ")");
        updateChannelName();
      }
    ...
    setInterval(function () {
      ...
      // Só manda mensagem no chat se tiver alguém na fila
      ...
      broadcast(
        ...
        "   " + names.join(", ") + "\n" +
        "ℹ️  Entre no canal 'Fila de espera' e use " + P + "entrar para participar!"
      );
    }, 30000);
  }
);
